...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($useremail_err) && empty($username_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        //prepare user_meta
        $setUserMetaSql = "INSERT INTO user_meta (useremail) VALUES (?)";
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                // NESTING WITH USERMETA 
                if ($stmt = mysqli_prepare($link, $setUserMetaSql)) {
                    mysqli_stmt_bind_param($stmt, "s", $param_useremail);
                    if (mysqli_stmt_execute($stmt)) {
...
<head>
    ...
    <title>Register | Priequity Portal</title>
